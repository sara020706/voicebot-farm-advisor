...
def load_model():
    ...
    if os.path.exists(MODEL_PATH):
        try:
            ...
            test = np.array([[90, 42, 43, 28, 82, 6.5, 202]])
            loaded.predict(test)
            model = loaded
            model_source = "pkl-model"
            logger.info(f"Loaded crop model from {MODEL_PATH}")
            return
        except Exception as e:
            logger.warning(f"Local model invalid: {e}")
    logger.info("No valid model found — using rule-based fallback")
    model = None
    model_source = "rule-based"
...
def rule_based_predict(N, P, K, ph, temperature, humidity, rainfall):
    ...
    for rule in CROP_RULES:
        score = sum([
            ...
            rule["P"][0]        <= P           <= rule["P"][1],
            rule["K"][0]        <= K           <= rule["K"][1],
            rule["ph"][0]       <= ph          <= rule["ph"][1],
            rule["temp"][0]     <= temperature <= rule["temp"][1],
            rule["humidity"][0] <= humidity    <= rule["humidity"][1],
            rule["rainfall"][0] <= rainfall    <= rule["rainfall"][1],
        ])
        scores.append((rule["crop"], round(score / 7, 4)))
    scores.sort(key=lambda x: x[1], reverse=True)
    best_crop, best_score = scores[0] if scores else ("rice", 0.5)
...
def predict(N, P, K, ph, temperature, humidity, rainfall):
    ...
    if model is not None:
        try:
            ...
            logger.info(f"ML model: {crop} {confidence}")
            return crop, confidence, "pkl-model"
        except Exception as e:
